use crate::tokenizer::{CssTokenizer, CssTokenType};
use crate::selector::{Selector, SelectorList, SimpleSelector, Combinator, AttributeOperator, PseudoClass, PseudoElement};
use crate::properties::{Property, PropertyValue, PropertyDeclaration, PropertyMap};
use crate::stylesheet::{Stylesheet, Rule, AtRule, MediaQuery};
...
impl CssParser {
    ...
    fn is_eof(&self) -> bool {
        self.position >= self.tokens.len() || matches!(self.current_token().token_type, CssTokenType::EOF)
    }
    ...
    fn parse_at_rule(&mut self) -> CssResult<Option<AtRule>> {
        ...
        if let CssTokenType::AtKeyword(name) = &self.current_token().token_type {
            ...
            while !self.is_eof() && !matches!(self.current_token().token_type, CssTokenType::LeftCurlyBracket | CssTokenType::Semicolon) {
            ...
            if matches!(self.current_token().token_type, CssTokenType::LeftCurlyBracket) {
                self.consume_token(); // consume '{'
                while !self.is_eof() && !matches!(self.current_token().token_type, CssTokenType::RightCurlyBracket) {
                    body.push(self.current_token().clone());
                    self.consume_token();
                }
                if matches!(self.current_token().token_type, CssTokenType::RightCurlyBracket) {
                    self.consume_token(); // consume '}'
                }
    ...
    fn parse_qualified_rule(&mut self) -> CssResult<Option<Rule>> {
        ...
        // Parse selector list
        while !self.is_eof() && !matches!(self.current_token().token_type, CssTokenType::LeftCurlyBracket) {
    ...
    fn parse_declaration(&mut self) -> CssResult<Option<PropertyDeclaration>> {
        if let CssTokenType::Ident(property_name) = &self.current_token().token_type {
            ...
            if matches!(self.current_token().token_type, CssTokenType::Colon) {
                ...
                // Parse value
                while !self.is_eof() && !matches!(self.current_token().token_type, CssTokenType::Semicolon | CssTokenType::RightCurlyBracket) {
    ...
    fn parse_value_from_tokens(&self, tokens: Vec<crate::tokenizer::CssToken>) -> CssResult<PropertyValue> {
        if tokens.is_empty() {
        ...
        match &tokens[0].token_type {
            ...
            CssTokenType::Dimension(n, unit) => {
                Ok(PropertyValue::Length(*n, crate::properties::LengthUnit::Px)) // Simplified
            }
    ...
    fn parse_selector_list_from_prelude(&self, prelude: Vec<crate::tokenizer::CssToken>) -> CssResult<SelectorList> {
