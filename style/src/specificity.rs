...
impl SpecificityUtils {
    ...
    pub fn calculate_simple_selector_specificity(selector: &css_parser::SimpleSelector) -> Specificity {
    ...
    pub fn specificity_to_string(specificity: &Specificity) -> String {
        format!("{},{},{},{}", specificity.a, specificity.b, specificity.c, specificity.d)
    }
    ...
    pub fn specificity_from_string(s: &str) -> Result<Specificity, String> {
        ...
        let a = parts[0].parse::<u32>().map_err(|_| "Invalid specificity value")?;
        let b = parts[1].parse::<u32>().map_err(|_| "Invalid specificity value")?;
        let c = parts[2].parse::<u32>().map_err(|_| "Invalid specificity value")?;
        let d = parts[3].parse::<u32>().map_err(|_| "Invalid specificity value")?;
    ...
    pub fn calculate_declaration_specificity_from_string(declaration: &str) -> Result<Specificity, String> {
